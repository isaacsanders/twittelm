...
Elm.Input.make = function (_elm) {
   ...
   var validateTweet = function (tweet) {
      return function () {
         ...
         {case "ContentTweet":
            return _U.cmp(0,
              $String.length(_v0._0.content)) < 0;
            case "Retweet":
            return function () {
                 switch (_v0._1.ctor)
                 {case "ContentTweet":
                    return !_U.eq(_v0._0,
                      _v0._1._0.username);
                    case "Retweet":
                    return !_U.eq(_v0._1._0,
                      _v0._0) && validateTweet(A2($Types.Retweet,
                      _v0._0,
                      _v0._1._1));}
                 _U.badCase($moduleName,
   ...
   var validateUsernameContent = function ($) {
      return $Basics.not(function ($) {
         return $Regex.contains($Regex.regex($));
      }("[^\\w_]")(function (_) {
         return _.string;
      }($)));
   };
   var WriteTweet = function (a) {
      return {ctor: "WriteTweet"
             ,_0: a};
   };
   var ChangeUsername = function (a) {
      return {ctor: "ChangeUsername"
             ,_0: a};
   };
   var NewTweet = function (a) {
      return {ctor: "NewTweet"
             ,_0: a};
   };
   ...
   var tweetsMailbox = $Signal.mailbox($Types.ContentTweet({_: {}
                                                           ,content: ""
                                                           ,username: ""}));
   var tweetContentMailbox = $Signal.mailbox($Graphics$Input$Field.noContent);
   var usernameMailbox = $Signal.mailbox($Graphics$Input$Field.noContent);
   var inputs = $Signal.mergeMany(_L.fromArray([A2($Signal._op["<~"],
                                               Layout,
                                               $Window.dimensions)
                                               ,A2($Signal._op["<~"],
                                               NewTweet,
                                               A3($Signal.filter,
                                               validateTweet,
                                               $Types.ContentTweet({_: {}
                                                                   ,content: ""
                                                                   ,username: ""}),
                                               tweetsMailbox.signal))
                                               ,A2($Signal._op["<~"],
                                               ChangeUsername,
                                               A3($Signal.filter,
                                               validateUsernameContent,
                                               $Graphics$Input$Field.noContent,
                                               usernameMailbox.signal))
                                               ,A2($Signal._op["<~"],
                                               WriteTweet,
                                               tweetContentMailbox.signal)]));
   _elm.Input.values = {_op: _op
                       ,inputs: inputs
                       ,usernameMailbox: usernameMailbox
                       ,tweetContentMailbox: tweetContentMailbox
                       ,tweetsMailbox: tweetsMailbox
                       ,Layout: Layout
                       ,NewTweet: NewTweet
...
Elm.State.make = function (_elm) {
   ...
   $Types = Elm.Types.make(_elm);
   var newState = {_: {}
                  ,tweetContent: $Graphics$Input$Field.noContent
                  ,tweets: _L.fromArray([])
                  ,usernameContent: $Graphics$Input$Field.noContent};
...
Elm.Types.make = function (_elm) {
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var originalUsername = function (tweet) {
      return function () {
         switch (tweet.ctor)
         {case "ContentTweet":
            return tweet._0.username;
            case "Retweet":
            return originalUsername(tweet._1);}
         _U.badCase($moduleName,
         "between lines 10 and 12");
      }();
   };
   var Retweet = F2(function (a,
   b) {
      return {ctor: "Retweet"
             ,_0: a
             ,_1: b};
   });
   var ContentTweet = function (a) {
      return {ctor: "ContentTweet"
             ,_0: a};
   };
   var TweetContent = F2(function (a,
   b) {
      return {_: {}
             ,content: b
             ,username: a};
   ...
   state) {
      return function () {
         ...
         return function () {
            ...
            switch (_v0.ctor)
            {case "ChangeUsername":
               return _U.replace([["usernameContent"
                                  ,_v0._0]],
                 state_);
               case "Layout": return state_;
               case "NewTweet":
               return _U.replace([["tweets"
                                  ,A2($List._op["::"],
                                  _v0._0,
                                  state_.tweets)]
                                 ,["tweetContent"
                 ...
                 state_);
               case "WriteTweet":
               return _U.replace([["tweetContent"
                                  ,_v0._0]],
                 state_);}
            _U.badCase($moduleName,
            "between lines 16 and 25");
         }();
      }();
   });
...
Elm.View.make = function (_elm) {
   ...
   $Text = Elm.Text.make(_elm),
   $Types = Elm.Types.make(_elm);
   var retweetButton = F2(function (username,
   tweet) {
      return function () {
         var message = A2($Signal.message,
         $Input.tweetsMailbox.address,
         A2($Types.Retweet,
         username,
         tweet));
   ...
   var tweetView = F2(function (currentUsername,
   tweet) {
      return function () {
         switch (tweet.ctor)
         {case "ContentTweet":
            return function () {
                 var rtButton = A2(retweetButton,
                 currentUsername,
                 tweet);
                 var visualContent = $Text.fromString(tweet._0.content);
                 var visualUsername = $Text.bold($Text.append($Text.fromString("@"))($Text.fromString(tweet._0.username)));
                 return A2($Graphics$Element.flow,
                 $Graphics$Element.down,
                 _L.fromArray([$Graphics$Element.leftAligned(visualUsername)
                              ,$Graphics$Element.leftAligned(visualContent)
                              ,rtButton]));
              }();
            case "Retweet":
            return function () {
                 var originalUsername = $Types.originalUsername(tweet._1);
                 var retweetedBy = $Text.italic($Text.append($Text.fromString("Retweeted by "))($Text.fromString(tweet._0)));
                 return A2($Graphics$Element.flow,
                 $Graphics$Element.down,
                 _L.fromArray([A2(tweetView,
                              originalUsername,
                              tweet._1)
                              ,$Graphics$Element.leftAligned(retweetedBy)]));
              }();}
         _U.badCase($moduleName,
         "between lines 51 and 70");
      }();
   });
   var sendTweetButton = F2(function (username,
   content) {
      return function () {
         var newTweet = $Types.ContentTweet({_: {}
                                            ,content: content
                                            ,username: username});
         var message = A2($Signal.message,
         $Input.tweetsMailbox.address,
   ...
   var tweetField = A3($Graphics$Input$Field.field,
   $Graphics$Input$Field.defaultStyle,
   $Signal.message($Input.tweetContentMailbox.address),
   "Write your tweet here...");
   var paragraphStyle = function () {
      var padding = $Graphics$Input$Field.defaultStyle.padding;
      var paragraphPadding = _U.replace([["right"
                                         ,padding.right * 2]
   ...
   var usernameField = A3($Graphics$Input$Field.field,
   $Graphics$Input$Field.defaultStyle,
   $Signal.message($Input.usernameMailbox.address),
   "Username");
   var renderState = function (state) {
      return function () {
         var usernameContent = A2($Debug.watch,
         "Username",
         state.usernameContent);
         var username = usernameContent.string;
         var tweetContent = state.tweetContent;
         var sendButton = A2(sendTweetButton,
         usernameContent.string,
         tweetContent.string);
         return $Graphics$Element.flow($Graphics$Element.down)(A2($List._op["::"],
         usernameField(state.usernameContent),
         A2($List._op["::"],
